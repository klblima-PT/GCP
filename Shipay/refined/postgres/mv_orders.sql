...
SELECT to_date(to_char(orders.created_at at TIME zone '-3', 'YYYY-MM-DD'), 'YYYY-MM-DD') AS orders_created_date
	,to_date(to_char(orders.expiration_date at TIME zone '-3', 'YYYY-MM-DD'), 'YYYY-MM-DD') AS orders_expiration_date
...
WHERE orders.created_at >= (to_date(to_char((current_timestamp at TIME zone '3') - interval '1 days', 'YYYY-MM-DD'), 'YYYY-MM-DDThh24:mi:ss') at TIME zone '-3')
...
CREATE INDEX idx_mv_recent_orders_created ON public.mv_recent_orders USING btree (orders_created_date);
CREATE INDEX idx_mv_recent_orders_payment ON public.mv_recent_orders USING btree (orders_payment_date);
CREATE INDEX idx_mv_recent_orders_psp ON public.mv_recent_orders USING btree (psp_provider);
CREATE INDEX idx_mv_recent_orders_status ON public.mv_recent_orders USING btree (order_status);
CREATE INDEX idx_mv_recent_orders_store_pos ON public.mv_recent_orders USING btree (store_pos_id);