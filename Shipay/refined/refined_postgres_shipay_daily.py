...
str_tables_list = models.Variable.get("postgres_tables_refined_shipay_daily")
...
def create_taskgroup(task_group_prefix, table_info):
    ...
    with TaskGroup(group_id=f"{task_group_prefix}_{table}") as tg1:
        ...
        create_table = _create_table(
            table, f"queries/refined/ddl/ddl_refined_shipay_{table}.sql"
        )
        populate_table = _populate_table(
            table, f"queries/refined/dml/dml_refined_shipay_{table}.sql"
        )
