...
str_tables_list = models.Variable.get("postgres_tables_raw_shipay_full_filter")
...
def create_taskgroup_last_date(task_group_prefix, table_info):
    ...
    from_date = table_info['from_date']
    to_date = table_info['to_date']
    ...
    with TaskGroup(group_id=f"{task_group_prefix}_{table}") as tg1:
        ...
        # Full + Filter >= Carga full + filter periodo com APPEND    
            ...
            query = f"SELECT {colunas_xcom} FROM {schema}.{table} WHERE cast({column_primary} as date) BETWEEN '{from_date}'  AND '{to_date}'"
...
with models.DAG(
    'raw_postgres_shipay_aws_full_filter',
