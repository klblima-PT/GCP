...
MERGE INTO `production-347022.trusted_shipay_aws.pos_products` AS bronze
USING (
  SELECT
    * EXCEPT (rnk)
  FROM (
...
dt_carga_raw,
	 ...
	 FROM
       `production-347022.raw_shipay_aws.pos_products`
  WHERE created_at_date = current_date()
  AND SAFE_CAST(updated_at AS DATETIME) BETWEEN DATETIME_SUB(DATETIME(current_datetime()), INTERVAL 2 HOUR) 
