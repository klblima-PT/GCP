...
MERGE INTO `production-347022.trusted_shipay.pos_vendors` AS bronze
USING (
  SELECT
    * EXCEPT (rnk)
  FROM (
...
, safe_cast(created_at as timestamp)  created_at  		
,safe_cast(updated_at as timestamp)  updated_at  	
,created_at_date	
,degree 			
,external_crm_id 
,dt_carga_raw ,
	 ...
	 FROM
        `production-347022.raw_shipay.pos_vendors`
...
bronze.created_at_date  		 = safe_cast(raw.created_at_date as date)  			,
