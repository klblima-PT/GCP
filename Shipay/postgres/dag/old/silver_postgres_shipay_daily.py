...
str_tables_list = models.Variable.get("bq_tables_silver_bi_rendimento")
...
def create_taskgroup(task_group_prefix, table_info):
    ...
    with TaskGroup(group_id=f"{task_group_prefix}_{table}") as tg1:
        ...
        create_table = _create_table(
            table, f"queries/silver_bi_rendimento/ddl/ddl_silver_bi_rendimento_{table}.sql"
        )
        populate_table = _populate_table(
            table, f"queries/silver_bi_rendimento/dml/dml_silver_bi_rendimento_{table}.sql"
        )
