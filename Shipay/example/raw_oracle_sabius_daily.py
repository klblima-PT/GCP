...
#DRIVER_BUCKET = environment_variables["oracle_driver"]
...
USER_NAME = get_secret(PROJECT, 'eng-data-postgres_user', 1)
USER_PASS = get_secret(PROJECT, 'eng-data-postgres_pass', 1)
...
def prepare_request_columns(table, data_type_exists):
    ...
    read_process_gcs_file = ReadProcessGCSFileOperator(
            ...
            gcs_file_path=f'us-east4-cps-shipay-data-pr-2109341e-bucket/dags/queries/raw/columns/{table}_columns.txt',
            table_name=table,
...
def create_taskgroup_last_date(task_group_prefix, table_info):
    ...
    with TaskGroup(group_id=f"{task_group_prefix}_{table}") as tg1:
        ...
        create_table = _create_table(
            table,
            f"queries/raw/ddl/ddl_raw_shipay_{table}.sql",
        )

        prepare_request_columns_operator = prepare_request_columns(table, data_type_exists)

        #delta = D-1, NÃƒO contempla os dados do dia da carga
        if type_table == "delta":
...
with models.DAG(
    ...
    trigger_bronze = TriggerDagRunOperator (task_id='trigger_bronze',trigger_dag_id = 'bronze_oracle_sabius', dag=dag)

start >> tg_group  >> end
trigger_bronze.set_upstream(end)