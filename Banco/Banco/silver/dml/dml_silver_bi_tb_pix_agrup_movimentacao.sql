...
-- ** tb_pix_agrup_movimentacao  **
-- DEVELOPED BY HVAR CONSULTING | DataOps team

MERGE INTO `corporativo-poc-hvar.silver_bi.tb_pix_agrup_movimentacao` AS bronze
USING (
  SELECT
    * EXCEPT (rnk)
  FROM (
...
valortop			,
	 ...
	 FROM
        `corporativo-poc-hvar.bronze_bi.bi_pix_mat_dashpixmovimentacao`
     --WHERE
